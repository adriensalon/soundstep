...
namespace {
    ...
    void _require_psa(psa_status_t status, std::string message)
    {
    ...
    void _require_mbedtls(int status, std::string message)
    {
        if (status != 0) {
            throw security_error(std::move(message) + ": " + std::to_string(status));
        }
    ...
    struct _certificate_writer_guard {
        _certificate_writer_guard()
        {
    ...
    char _hex_digit(unsigned value)
    {
        return static_cast<char>(value < 10 ? '0' + value : 'a' + value - 10);
    }

    std::string _hex_fingerprint(const std::array<unsigned char, 32>& digest)
    {
        std::string _result;
        _result.resize(digest.size() * 2);
        for (std::size_t _index = 0; _index < digest.size(); ++_index) {
            _result[_index * 2] = _hex_digit(digest[_index] >> 4);
            _result[_index * 2 + 1] = _hex_digit(digest[_index] & 0x0f);
        }
        return _result;
    }
...
std::string data_fingerprint(std::string_view bytes)
{
    ...
    return _hex_fingerprint(_digest);
}
