...
namespace {
    ...
    void _draw_extension_badge(audio_extension extension)
    {
        ...
        const ImVec2 _size(_text_size.x + _padding.x * 2.0f, _text_size.y + _padding.y * 2.0f);
        const ImVec2 _position = ImGui::GetCursorScreenPos();
        ImDrawList* _draw = ImGui::GetWindowDrawList();
        _draw->AddRectFilled(_position, ImVec2(_position.x + _size.x, _position.y + _size.y), ImGui::GetColorU32(_background), ImGui::GetStyle().FrameRounding);
        _draw->AddText(ImVec2(_position.x + _padding.x, _position.y + _padding.y), ImGui::GetColorU32(_foreground), _label.c_str());
        ImGui::Dummy(_size);
    }

    void _draw_seek_bar(context& ctx, const playback_status& playback, double duration_seconds, float width, float height)
    {
        ...
        const bool _can_seek = playback.has_source && duration_seconds > 0.0 && std::isfinite(duration_seconds);
        const ImVec2 _position = ImGui::GetCursorScreenPos();
        ...
        if (_can_seek && ImGui::IsItemActive()) {
            const float _ratio = (std::clamp)((ImGui::GetIO().MousePos.x - _position.x) / width, 0.0f, 1.0f);
        ...
        const double _display_seconds = _seeking || _seek_committed ? _preview_seconds : playback.position_seconds;
        const float _played_target = _can_seek ? static_cast<float>((std::clamp)(_display_seconds / duration_seconds, 0.0, 1.0)) : 0.0f;
        const float _buffered_target = _can_seek ? static_cast<float>((std::clamp)((playback.position_seconds + playback.buffered_seconds) / duration_seconds, 0.0, 1.0)) : 0.0f;
        const double _maximum_continuous_step = (std::max)(0.25, static_cast<double>(ImGui::GetIO().DeltaTime) * 4.0);
        const bool _duration_changed = _played_position_initialized && std::abs(duration_seconds - _last_duration_seconds) > 0.01;
        const bool _position_jumped = _played_position_initialized && std::abs(_display_seconds - _last_display_seconds) > _maximum_continuous_step;
        ...
        const float _played_ratio = _snap_played_position
            ...
            : animation_tween(_motion_owner, 0x21001u, _played_target, 0.12f, iam_ease_out_cubic);
        if (_can_seek) {
        ...
        const float _buffered_ratio = animation_tween(_motion_owner, 0x21002u, _buffered_target, animation_relaxed, iam_ease_out_cubic);
        const bool _hovered = ImGui::IsItemHovered();
        const bool _active = ImGui::IsItemActive();
        const float _thumb_reveal = animation_tween(_motion_owner, 0x21003u, (_hovered || _active) && _can_seek ? 1.0f : 0.0f, animation_quick, iam_ease_out_cubic);
        ...
        if (_buffered_ratio > 0.0f) {
            _draw->AddRectFilled(_position, ImVec2(_position.x + width * _buffered_ratio, _end.y), ImGui::GetColorU32(ImGuiCol_ButtonHovered), _rounding);
        }
        if (_played_ratio > 0.0f) {
            _draw->AddRectFilled(_position, ImVec2(_position.x + width * _played_ratio, _end.y), ImGui::GetColorU32(ImGuiCol_CheckMark), _rounding);
        }
        if (_thumb_reveal > 0.001f) {
            const ImVec2 _thumb_position(_position.x + width * _played_ratio, _position.y + height * 0.5f);
            _draw->AddCircleFilled(_thumb_position, height * (0.8f + 0.38f * _thumb_reveal), animation_with_alpha(ImGui::GetStyleColorVec4(ImGuiCol_CheckMark), 0.14f * _thumb_reveal));
            _draw->AddCircleFilled(_thumb_position, height * (0.42f + 0.38f * _thumb_reveal), animation_with_alpha(ImGui::GetStyleColorVec4(ImGuiCol_CheckMark), _thumb_reveal));
        }
    }

    bool _draw_transport_button(const char* label, ImVec2 size, bool enabled, ImGuiID channel)
    {
        ...
        const float _hover = animation_tween(_owner, channel, _hovered ? 1.0f : 0.0f, animation_quick, iam_ease_out_cubic);
        const ImVec4 _button_color = iam_get_blended_color(ImGui::GetStyleColorVec4(ImGuiCol_Button), ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered), _hover, iam_col_oklab);
    ...
    bool _draw_primary_button(const char* label, ImVec2 size, bool enabled, bool active, bool buffering)
    {
        ...
        const ImVec2 _position = ImGui::GetCursorScreenPos();
        const bool _hovered = enabled && ImGui::IsMouseHoveringRect(_position, ImVec2(_position.x + size.x, _position.y + size.y));
        const float _hover = animation_tween(_owner, 0x22001u, _hovered ? 1.0f : 0.0f, animation_quick, iam_ease_out_cubic);
        const float _active = animation_tween(_owner, 0x22002u, active ? 1.0f : 0.0f, animation_normal, iam_ease_out_cubic);
        const float _pulse = buffering ? iam_oscillate(_owner, 0.055f, 0.75f, iam_wave_sine, 0.0f, ImGui::GetIO().DeltaTime) : 0.0f;
        ...
        ImVec4 _background = iam_get_blended_color(_idle_color, _playing_color, _active, iam_col_oklab);
        const ImVec4 _hover_color = active ? ImVec4(0.58f, 0.44f, 0.20f, 1.0f) : ImGui::GetStyleColorVec4(ImGuiCol_SliderGrab);
...
void draw_player(context& ctx)
{
    ...
    if (ctx.current_track) {
        ...
        const float _cover_reveal = animation_tween(_cover_motion_owner, 0x23001u, 1.0f, animation_relaxed, iam_ease_out_cubic);
        ImGui::PopID();
        ...
        if (_cover) {
            ImGui::GetWindowDrawList()->AddImageRounded(reinterpret_cast<ImTextureID>(_cover->native_id), _animated_cover_min, _animated_cover_max, ImVec2(0.0f, 0.0f), ImVec2(1.0f, 1.0f), animation_with_alpha(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), _cover_reveal), _rounding);
        } else {
            ImGui::GetWindowDrawList()->AddRectFilled(_animated_cover_min, _animated_cover_max, animation_with_alpha(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), _cover_reveal), _rounding);
        }
    ...
    if (ctx.current_track) {
        ...
        ImGui::PushClipRect(_panel_screen_start, ImVec2(_panel_screen_start.x + (_controls_x - _panel_start.x) - _transport_spacing, _panel_screen_start.y + _cover_size), true);
    ...
    const double _duration_seconds = ctx.current_track && ctx.current_track->duration_ms != 0 ? static_cast<double>(ctx.current_track->duration_ms) / 1000.0 : _playback.duration_seconds;
    const double _elapsed_seconds = _duration_seconds > 0.0 ? (std::clamp)(_playback.position_seconds, 0.0, _duration_seconds) : _playback.position_seconds;
    const std::string _elapsed = _time_text(_elapsed_seconds);
    const std::string _duration = _duration_seconds > 0.0 ? _time_text(_duration_seconds) : "--:--";
    ...
    const bool _show_times = _panel_width >= _elapsed_width + _duration_width + _minimum_seek_width + _style.ItemSpacing.x * 2.0f;
    const float _seek_width = (std::max)(_minimum_seek_width, _panel_width - (_show_times ? _elapsed_width + _duration_width + _style.ItemSpacing.x * 2.0f : 0.0f));
    ...
    if (_draw_transport_button(icons::previous, ImVec2(_button_width, _control_height), _can_previous, 0x24001u)) {
    ...
    ImGui::SameLine();
    if (_draw_primary_button(_primary_label, ImVec2(_button_width, _control_height), _playback.has_source, _is_active, _playback.state == playback_state::buffering)) {
        if (_is_active) {
    ...
    if (_draw_transport_button(icons::next, ImVec2(_button_width, _control_height), _can_next, 0x24002u)) {
        play_next_track(ctx);
    }
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
        draw_tooltip("Next");
    ...
    const float _seek_x = _panel_start.x + (_show_times ? _elapsed_width + _style.ItemSpacing.x : 0.0f);
    const float _seek_y = _progress_row_y + (ImGui::GetTextLineHeight() - _seek_height) * 0.5f + 2.0f;
    ImGui::SetCursorPos(ImVec2(_seek_x, _seek_y));
    _draw_seek_bar(ctx, _playback, _duration_seconds, _seek_width, _seek_height);
    if (_show_times) {
        ImGui::SetCursorPos(ImVec2(_panel_start.x + _panel_width - _duration_width, _progress_row_y));
